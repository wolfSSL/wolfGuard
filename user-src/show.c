...
static char *public_key(const uint8_t key[static WG_PUBLIC_KEY_LEN])
{
	static char base64[WG_BASE64_LEN(WG_PUBLIC_KEY_LEN)];

	if (! wg_to_base64(base64, sizeof(base64), key, WG_PUBLIC_KEY_LEN))
		return "(error)";
	return base64;
}
