...
del_if() {
	...
	if [[ -z $TABLE || $TABLE == auto ]] && get_fwmark table && [[ $(wg-fips show "$INTERFACE" allowed-ips) =~ /0(\ |$'\n'|$) ]]; then
...
set_mtu_up() {
	...
	done < <(wg-fips show "$INTERFACE" endpoints)
	if [[ $mtu -eq 0 ]]; then
...
get_fwmark() {
	local fwmark
	fwmark="$(wg-fips show "$INTERFACE" fwmark)" || return 1
...
add_default() {
	...
	if ! get_fwmark table; then
		...
		cmd wg-fips set "$INTERFACE" fwmark $table
	fi
...
set_config() {
	cmd wg-fips setconf "$INTERFACE" <(echo "$WG_CONFIG")
}
...
save_config() {
	...
	old_umask="$(umask)"
	umask 077
	current_config="$(cmd wg-fips showconf "$INTERFACE")"
...
cmd_up() {
	...
	set_mtu_up
	set_dns
	for i in $(while read -r _ i; do for i in $i; do [[ $i =~ ^[0-9a-z:.]+/[0-9]+$ ]] && echo "$i"; done; done < <(wg-fips show "$INTERFACE" allowed-ips) | sort -nr -k 2 -t /); do
		add_route "$i"
	done
...
cmd_down() {
	[[ " $(wg-fips show interfaces) " == *" $INTERFACE "* ]] || die "\`$INTERFACE' is not a WolfGuard interface"
